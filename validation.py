...
class Validator:
    ...
    def validate_point_id(point: Any, field_name: str = "point") -> str:
        ...
        if not point_str:
            raise ValidationError(
                "El ID del punto no puede estar vacío",
                field=field_name,
                details={"min_length": 1}
            )
    ...
    def validate_run_index(run: Any, field_name: str = "run") -> int:
        ...
        try:
            ...
            raise ValidationError(
                "El índice de repetición debe ser un número entero",
                field=field_name,
    ...
    def validate_parallel_streams(parallel: Any, field_name: str = "parallel") -> int:
        ...
        try:
            ...
            raise ValidationError(
                "El número de streams paralelos debe ser un número entero",
                field=field_name,
                details={"type": "integer"}
            ) from exc
        ...
        if parallel_int < Validator.PARALLEL_MIN or parallel_int > Validator.PARALLEL_MAX:
            raise ValidationError(
                f"El número de streams paralelos debe estar entre {Validator.PARALLEL_MIN} y "
                f"{Validator.PARALLEL_MAX}",
                field=field_name,
    ...
    def validate_repeats(repeats: Any, field_name: str = "repeats") -> int:
        ...
        try:
            ...
            raise ValidationError(
                "El número de repeticiones debe ser un número entero",
                field=field_name,
                details={"type": "integer"}
            ) from exc
    ...
    def validate_points_list(points: Any, field_name: str = "points") -> List[str]:
        ...
        if not isinstance(points, list):
            raise ValidationError(
                "Los puntos deben ser una lista o cadena de texto separada por espacios",
                field=field_name,
        ...
        if not points:
            raise ValidationError(
                "Debe proporcionar al menos un punto para la encuesta",
                field=field_name,
                details={"min_count": 1}
            )
    ...
    @staticmethod
    def validate_run_point_payload(
        ...
        validated = {}
        validated["device"] = Validator.validate_device_name(
            payload.get("device", defaults.get("device", "phone"))
        )
        ...
        validated["parallel"] = Validator.validate_parallel_streams(
            payload.get("parallel", defaults.get("parallel", 4))
        )

        return validated

    @staticmethod
    def validate_start_survey_payload(
        payload: Dict[str, Any],
        defaults: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
